...
int length_linked_list ( Node *head_pp)
{
    int node_count = 0;
    while (head_pp)
    {
...
void reverse(Node **head_pp) {
    ...
    if(!node_pointers){perror("Coudl no allocate memory for reverse. \n");}exit;

    Node *curr_node_pointer = *head_pp;
    int curr_node_count=0;
    while (curr_node_pointer)
    {
        node_pointers[curr_node_count++] = curr_node_pointer;
        curr_node_pointer = curr_node_pointer->next;
    }
    *head_pp = node_pointers[curr_node_count-1];

   for (int i = curr_node_count-1 ; i > 0; i--)
   {
